<template>
  <v-container>
    <v-card>
      <v-card-text>
        <p>
          The <code>mounted</code> hook runs right after the initial render
          happens and after the template or render functions have been compiled.
          you will have full access to the reactive component, templates, and
          rendered DOM (via <code>this.$el</code>).
          <span class="text-decoration-underline"
            >Use the <code>mounted</code> hook for modifying the
            DOM—particularly when integrating non-Vue libraries:</span
          >
        </p>
...
export default {
  ...
  // when our Vue app is mounted, go get the list of users
  mounted: function() {
    this.getUsers();
  }
}
